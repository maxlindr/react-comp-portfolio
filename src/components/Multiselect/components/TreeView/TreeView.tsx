...
import { TreeNode, noop, initExpanded, recalculateIds } from './utils';
...
const TreeView = <T extends string | number>({
  ...
  const [expanded, setExpanded] = useState<T[]>(() =>
    initExpanded(selected, data)
  );

  const handleNodeCheck = (id: T, checked: boolean) => {
    onChange(recalculateIds(selected, id, checked));
  };
